...
const fileSystem = new MockFileSystem('C:/NovaProjects');
const projectService = new ProjectService(fileSystem, 'C:/NovaProjects');
...
export default function App() {
  ...
  function createProject() {
    ...
    }).catch((caught: unknown) => setError(caught instanceof Error ? caught.message : 'Projekt konnte nicht erstellt werden.'));
  }

  async function openProject(selected: RecentProject) { try { await projectService.readManifest(selected.path); projectService.rememberProject(selected); setProject(selected); setView('workspace'); } catch (caught) { setError(caught instanceof Error ? caught.message : 'Projekt konnte nicht geöffnet werden.'); } }
  ...
  return <div className="app-shell">
    ...
    <div className="body-layout">
      ...
      {view === 'home' ? <Home projects={projects} onCreate={createProject} onOpen={openProject} /> : view === 'settings' ? <SettingsPage config={config} onConfigChange={updateConfig} /> : <Workspace project={project} config={config} bottomPanel={bottomPanel} setBottomPanel={setBottomPanel} />}
    </div>
  </div>;
}
